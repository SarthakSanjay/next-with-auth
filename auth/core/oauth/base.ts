...
export class OAuthClient<T> {
  ...
  private get redirectUrl() {
    return new URL(this.provider.toLowerCase(), env.OAUTH_REDIRECT_URL_BASE);
  }

  createAuthUrl(cookies: Pick<Cookies, "set">) {
    const state = createState(cookies);
    const codeVerifier = createCodeVarifier(cookies);
...
function createCodeVarifier(cookies: Pick<Cookies, "set">) {
